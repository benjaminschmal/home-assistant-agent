...
BACKENDS = [
    ...
    {"id": "salesforce", "name": "Salesforce", "available": False, "status": "Connector prepared"},
]
PROVIDERS = [
    {"id": "openai", "name": "OpenAI / GPT", "available": True},
    {"id": "ollama", "name": "Ollama (local)", "available": False, "status": "Connector prepared"},
]
...
def validate_setup(request: SetupRequest):
    if request.ai_provider not in {p["id"] for p in PROVIDERS}:
        raise ValueError("Unknown AI provider")
    if request.backend not in {b["id"] for b in BACKENDS}:
        raise ValueError("Unknown backend")
    config = load_config()
    if request.ai_provider == "openai" and not request.openai_api_key and not config.get("openai_api_key"):
        raise ValueError("OpenAI API key is required")
    if request.ai_provider == "ollama":
        raise ValueError("Ollama connector is prepared but not enabled yet")
    if request.backend == "homeassistant" and not request.mcp_url and not config.get("homeassistant", {}).get("mcp_url"):
        raise ValueError("MCP URL is required for Home Assistant")
    # Salesforce may already be selected and configured before its connector is enabled.

SETUP_HTML = r'''<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Agent Setup</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f5f5f5;margin:0}.card{max-width:720px;margin:35px auto;background:#fff;padding:28px;border-radius:16px;box-shadow:0 4px 20px #0001}h1{margin-top:0}.grid{display:grid;gap:14px}label{font-weight:600}input,select{box-sizing:border-box;width:100%;padding:12px;border:1px solid #ccc;border-radius:8px;font-size:15px}button{padding:12px 18px;border:0;border-radius:8px;background:#1976d2;color:white;font-size:15px;cursor:pointer}.hidden{display:none}.hint{font-size:13px;color:#666}.error{color:#b00020;margin-top:10px}.section{border-top:1px solid #eee;padding-top:18px;margin-top:18px}</style></head><body><div class="card"><h1>AI Agent Setup</h1><p class="hint">KI-Provider und Backend werden hier ausgewählt. Salesforce ist bereits vorbereitet; der Connector wird aktiviert, sobald die Salesforce-Zugänge vorhanden sind.</p><div class="grid"><label>KI-Provider</label><select id="provider"><option value="openai">OpenAI / GPT</option><option value="ollama" disabled>Ollama (später)</option></select><label>Backend</label><select id="backend"><option value="homeassistant">Home Assistant</option><option value="salesforce">Salesforce (vorbereitet)</option></select><div id="openai" class="section"><label>OpenAI API Key</label><input id="key" type="password" placeholder="sk-..."><label>Modell</label><input id="model" value="gpt-5"></div><div id="ha" class="section"><label>MCP URL</label><input id="mcp" placeholder="http://home-assistant-mcp:8000/mcp"><label>Home Assistant URL</label><input id="haurl" placeholder="http://homeassistant:8123"><label>Home Assistant Token</label><input id="hatoken" type="password"></div><div id="sf" class="section hidden"><p><strong>Salesforce</strong></p><p class="hint">Die Zugangsdaten können bereits gespeichert werden. Die eigentliche Salesforce-Tool-/Metadata-Schicht folgt als nächster Entwicklungsschritt.</p><label>Instance URL</label><input id="sfurl" placeholder="https://your-org.my.salesforce.com"><label>Client ID</label><input id="sfid"><label>Client Secret</label><input id="sfsecret" type="password"></div><button id="save">Setup speichern</button><div id="error" class="error"></div></div></div><script>const provider=document.getElementById('provider'),backend=document.getElementById('backend'),sf=document.getElementById('sf'),ha=document.getElementById('ha');function toggle(){sf.classList.toggle('hidden',backend.value!=='salesforce');ha.classList.toggle('hidden',backend.value!=='homeassistant')}backend.onchange=toggle;provider.onchange=toggle;toggle();document.getElementById('save').onclick=async()=>{const error=document.getElementById('error');error.textContent='';const body={ai_provider:provider.value,backend:backend.value,openai_api_key:document.getElementById('key').value,openai_model:document.getElementById('model').value,mcp_url:document.getElementById('mcp').value,ha_url:document.getElementById('haurl').value,ha_token:document.getElementById('hatoken').value,salesforce_instance_url:document.getElementById('sfurl').value,salesforce_client_id:document.getElementById('sfid').value,salesforce_client_secret:document.getElementById('sfsecret').value};try{const r=await fetch('/api/setup',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const d=await r.json();if(!r.ok)throw Error(d.error||'Setup fehlgeschlagen');location.href='/'}catch(e){error.textContent=e.message}};</script></body></html>'''

CHAT_HTML = r'''<!doctype html><html lang="de"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>AI Agent</title><style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;background:#f5f5f5;margin:0}.card{max-width:900px;margin:35px auto;background:#fff;padding:24px;border-radius:16px;box-shadow:0 4px 20px #0001}h1{margin-top:0}.bar{display:flex;gap:10px;align-items:center;margin-bottom:14px}.badge{padding:7px 10px;border-radius:8px;background:#eee;font-size:13px}.link{margin-left:auto}.chat{min-height:350px;max-height:600px;overflow:auto;border:1px solid #ddd;border-radius:10px;padding:15px}.msg{margin:10px 0;padding:11px 13px;border-radius:10px;white-space:pre-wrap}.user{background:#e8f0fe}.assistant{background:#f0f0f0}.row{display:flex;gap:10px;margin-top:14px}input{flex:1;padding:13px;border:1px solid #ccc;border-radius:8px;font-size:16px}button{padding:13px 18px;border:0;border-radius:8px;background:#1976d2;color:white}</style></head><body><div class="card"><div class="bar"><h1>AI Agent</h1><span class="badge" id="provider"></span><span class="badge" id="backend"></span><a class="link" href="/setup">Settings</a></div><div id="chat" class="chat"></div><div class="row"><input id="message" placeholder="Nachricht ..."><button id="send">Senden</button></div></div><script>const chat=document.getElementById('chat'),input=document.getElementById('message'),send=document.getElementById('send');function add(t,c){const d=document.createElement('div');d.className='msg '+c;d.textContent=t;chat.appendChild(d);chat.scrollTop=chat.scrollHeight}async function init(){const r=await fetch('/api/config');const c=await r.json();document.getElementById('provider').textContent=c.ai_provider;document.getElementById('backend').textContent=c.backend}async function go(){const m=input.value.trim();if(!m)return;add(m,'user');input.value='';send.disabled=true;add('Denke nach ...','assistant');try{const r=await fetch('/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:m,model:'openai'})});const d=await r.json();chat.lastChild.remove();add(r.ok&&!d.error?d.response:'Fehler: '+(d.error||'Unbekannter Fehler'),'assistant')}catch(e){chat.lastChild.remove();add('Verbindungsfehler: '+e,'assistant')}finally{send.disabled=false;input.focus()}}send.onclick=go;input.onkeydown=e=>{if(e.key==='Enter')go()};init();input.focus();</script></body></html>'''
...
async def models():
    config=load_config()
    return {"default_model":config.get("openai_model","gpt-5"),"provider":config.get("ai_provider","openai"),"providers":PROVIDERS,"backends":BACKENDS}
...
async def run_agent(session,tools,user_message:str):
    ...
    messages=[{'role':'system','content':'You are an AI agent. Use available backend tools. Never invent IDs or current values. For configuration changes, inspect current configuration first and make the smallest requested change. Never claim a write succeeded unless the tool confirms success.'},{'role':'user','content':user_message}]
    ...
    for _ in range(int(config['agent']['max_tool_rounds'])):
        ...
        for call in message.tool_calls:
            ...
            result=await run_timeout(session.call_tool(call.function.name,args),float(config['agent']['mcp_timeout_seconds']),f'Backend tool {call.function.name}')
...
async def chat(request:ChatRequest):
    ...
    if config.get('backend')=='salesforce':return {'error':'Salesforce backend is selected, but the connector is not enabled yet. Configure Salesforce access and enable the connector in the next step.'}
