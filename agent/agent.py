...
async function loadVersion(){try{const response=await fetch("/version"),data=await response.json();let text=`Agent v${data.agent_version} · MCP v${data.mcp_version||"unbekannt"}`;if(data.public_update_available)text+=` · ⚠ Neue Version verfügbar: Agent v${data.public_agent_version}`;document.getElementById("versionInfo").textContent=text}catch(error){document.getElementById("versionInfo").textContent="Agent-Version nicht verfügbar · MCP-Version nicht verfügbar"}}
...
async def chat(request:ChatRequest):
    ...
    try:
        ...
        logger.exception("Unexpected agent request failed")
