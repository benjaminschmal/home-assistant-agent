...
if not HA_URL: raise RuntimeError("HA_URL is not configured")
if not HA_TOKEN: raise RuntimeError("HA_TOKEN is not configured")
if not 1 <= MAX_SEARCH_RESULTS <= 200: raise RuntimeError("MAX_SEARCH_RESULTS must be between 1 and 200")

def auth_headers() -> dict[str,str]: return {"Authorization": f"Bearer {HA_TOKEN}", "Content-Type": "application/json"}

async def ha_get(path: str) -> Any:
    try:
        async with httpx.AsyncClient(timeout=HA_TIMEOUT) as client:
            response = await client.get(f"{HA_URL}{path}", headers=auth_headers()); response.raise_for_status(); return response.json()
    except httpx.TimeoutException as exc: raise RuntimeError(f"Home Assistant request timed out after {HA_TIMEOUT:.0f}s") from exc
    except httpx.HTTPStatusError as exc: raise RuntimeError(f"Home Assistant returned HTTP {exc.response.status_code}") from exc
    except httpx.HTTPError as exc: raise RuntimeError("Home Assistant request failed") from exc

async def ha_post(path: str, payload: dict[str,Any]) -> Any:
    try:
        async with httpx.AsyncClient(timeout=HA_TIMEOUT) as client:
            response = await client.post(f"{HA_URL}{path}", headers=auth_headers(), json=payload); response.raise_for_status(); return response.json()
    except httpx.TimeoutException as exc: raise RuntimeError(f"Home Assistant request timed out after {HA_TIMEOUT:.0f}s") from exc
...
async def ha_delete(path: str) -> Any:
    async with httpx.AsyncClient(timeout=HA_TIMEOUT) as client:
        response = await client.delete(f"{HA_URL}{path}", headers=auth_headers()); response.raise_for_status()
...
async def ha_call_service(domain: str, service: str, service_data: dict[str,Any]) -> Any:
    ...
    if not isinstance(service_data,dict): raise ValueError("service_data must be an object")
    entity_ids=service_data.get("entity_id")
    if entity_ids is not None:
        if isinstance(entity_ids,str): entity_ids=[entity_ids]; service_data={**service_data,"entity_id":entity_ids}
        if not isinstance(entity_ids,list) or not entity_ids: raise ValueError("entity_id must be a non-empty string or list")
        for entity_id in entity_ids:
            if not isinstance(entity_id,str) or not re.fullmatch(r"[a-z0-9_]+\.[a-z0-9_]+",entity_id): raise ValueError("Invalid entity_id in service_data")
...
async def ha_ws_command(command_type: str, payload: dict[str,Any]|None=None) -> Any:
    ...
    async with websockets.connect(ws_url,open_timeout=HA_TIMEOUT,close_timeout=HA_TIMEOUT) as websocket:
        ...
        message={"id":1,"type":command_type}; message.update(payload or {}); await websocket.send(json.dumps(message))
        while True:
            response=json.loads(await asyncio.wait_for(websocket.recv(),timeout=HA_TIMEOUT))
            if response.get("id")==1:
                if not response.get("success"):
                    error=response.get("error",{}); raise RuntimeError(f"Home Assistant WebSocket command failed: {error.get('message',command_type)}")
                return response.get("result")

async def list_ha_services() -> list[dict[str,Any]]:
    result=await ha_ws_command("get_services"); services=[]
    for domain,items in (result or {}).items():
        for service,definition in (items or {}).items(): services.append({"service":f"{domain}.{service}","name":definition.get("name") if isinstance(definition,dict) else None,"description":definition.get("description") if isinstance(definition,dict) else None})
...
async def create_dashboard(url_path:str,title:str,icon:str="mdi:view-dashboard",show_in_sidebar:bool=True,require_admin:bool=False)->Any:
    if "-" not in url_path: raise ValueError("Dashboard url_path must contain a hyphen, e.g. printer-dashboard")
...
async def delete_dashboard(dashboard_id:str)->Any:
    if not dashboard_id.strip(): raise ValueError("dashboard_id is required")
...
async def start_config_flow(handler:str,user_input:dict[str,Any]|None=None)->dict[str,Any]:
    if not re.fullmatch(r"[a-z0-9_]+",handler): raise ValueError("Invalid integration domain")
    payload={"handler":handler}
    if user_input is not None: payload["data"]=user_input
...
async def submit_config_flow(flow_id:str,user_input:dict[str,Any])->dict[str,Any]:
    if not flow_id.strip(): raise ValueError("flow_id is required")
    if not isinstance(user_input,dict): raise ValueError("user_input must be an object")
...
def config_file_path(filename:str)->Path:
    ...
    if path.parent!=CONFIG_ROOT: raise PermissionError("Invalid configuration path")
    return path

def backup_path(path:Path)->Path: return path.with_name(path.name+".agent-backup")
def validate_yaml(text:str)->None:
    try:
        ...
        def construct_tag(loader,tag_suffix,node):
            ...
            return None
        HomeAssistantLoader.add_multi_constructor("!",construct_tag); yaml.load(text,Loader=HomeAssistantLoader)
    except ImportError as exc: raise RuntimeError("PyYAML is required for configuration validation") from exc
...
def update_config_file(filename:str,content:str)->dict[str,Any]:
    ...
    path.write_text(content,encoding="utf-8"); return {"success":True,"file":filename,"backup":backup_path(path).name,"validated":True}

def normalize(value:Any)->str:
    value="" if value is None else str(value); value=value.casefold().replace("ä","ae").replace("ö","oe").replace("ü","ue").replace("ß","ss")
...
def search_score(query:str,entity:dict[str,Any],registry:str="")->int:
    terms=normalize(query).split(); searchable={k:normalize(entity.get(k,"")) for k in ("friendly_name","entity_id","device_class","state","domain")}; searchable["registry"]=normalize(registry); score=0
    for term in terms:
...
async def build_entity_index()->list[dict[str,Any]]:
    states,registries=await asyncio.gather(ha_get("/api/states"),ha_ws_command("config/entity_registry/list")); devices=await ha_ws_command("config/device_registry/list")
    by_device={d.get("id"):d for d in devices or [] if d.get("id")}; by_entity={e.get("entity_id"):e for e in registries or [] if e.get("entity_id")}; indexed=[]
    for state in states:
        entity_id=state.get("entity_id",""); attrs=state.get("attributes",{}) or {}; entry=by_entity.get(entity_id,{}); device=by_device.get(entry.get("device_id"))
...
async def list_tools(context,params)->ListToolsResult:
    tools=[
      ...
      Tool(name="create_dashboard",description="Create a storage-mode Lovelace dashboard.",inputSchema={"type":"object","properties":{"url_path":{"type":"string"},"title":{"type":"string"},"icon":{"type":"string"},"show_in_sidebar":{"type":"boolean"},"require_admin":{"type":"boolean"}},"required":["url_path","title"]}),
      ...
      Tool(name="list_config_entries",description="List configured Home Assistant integrations/config entries. Use before adding an integration to detect existing entries.",inputSchema={"type":"object","properties":{"domain":{"type":"string","description":"Optional integration domain, e.g. mqtt."}}}),
      Tool(name="start_integration_setup",description="Start a Home Assistant integration config flow, equivalent to beginning Add Integration in the UI. Use list_config_entries first. Returns the live form/schema and flow_id. Do not invent credentials or sensitive values.",inputSchema={"type":"object","properties":{"domain":{"type":"string"},"user_input":{"type":"object","description":"Optional values only when the live flow requires/accepts them."}},"required":["domain"]}),
      Tool(name="submit_integration_setup",description="Submit one step of an active Home Assistant integration config flow using its live flow_id and user_input. If another form is returned, inspect it and continue. Never invent passwords, tokens, certificates, or other secrets.",inputSchema={"type":"object","properties":{"flow_id":{"type":"string"},"user_input":{"type":"object"}},"required":["flow_id","user_input"]}),
...
async def call_tool(context,params)->CallToolResult:
    ...
    if params.name=="call_service": return text_result({"success":True,"result":await ha_call_service(str(args.get("domain","")).strip().lower(),str(args.get("service","")).strip().lower(),args.get("service_data",{}))})
    if params.name=="configuration_status": return text_result({"enabled":ALLOW_CONFIGURATION,"allowed_files":sorted(ALLOWED_CONFIG_FILES),"dashboard_management":ALLOW_CONFIGURATION,"integration_config_flows":ALLOW_CONFIGURATION})
    if params.name=="read_config": return text_result(read_config_file(args.get("filename","")))
    if params.name=="update_config": return text_result(update_config_file(args.get("filename",""),args.get("content","")))
    ...
    if (params.name in dashboard_tools or params.name in flow_tools) and not ALLOW_CONFIGURATION: raise PermissionError("Configuration management is disabled. Enable MCP_ALLOW_CONFIGURATION=true")
    if params.name=="list_dashboards": return text_result(await list_dashboards())
    if params.name=="read_dashboard": return text_result(await get_dashboard(args.get("url_path")))
    if params.name=="create_dashboard": return text_result({"success":True,"dashboard":await create_dashboard(str(args["url_path"]).strip(),str(args["title"]).strip(),str(args.get("icon") or "mdi:view-dashboard"),bool(args.get("show_in_sidebar",True)),bool(args.get("require_admin",False)))})
    ...
    if params.name=="delete_dashboard": return text_result({"success":True,"result":await delete_dashboard(str(args["dashboard_id"]).strip()),"dashboard_id":str(args["dashboard_id"]).strip()})
    ...
    if params.name=="start_integration_setup":
        domain=str(args["domain"]).strip().lower(); existing=await list_config_entries()
        matches=[e for e in existing if str(e.get("domain","")).lower()==domain]
        result=await start_config_flow(domain,args.get("user_input"))
        if matches: result={"warning":"An existing config entry for this integration was found. Verify whether the user really wants another instance.","existing_entries":[{k:e.get(k) for k in ("entry_id","domain","title","source","state") if k in e} for e in matches],"flow":result}
        return text_result(result)
    if params.name=="submit_integration_setup": return text_result(await submit_config_flow(str(args["flow_id"]),args.get("user_input",{})))
    if params.name=="abort_integration_setup": return text_result(await abort_config_flow(str(args["flow_id"])))
    raise ValueError(f"Unknown tool: {params.name}")

server=Server("home-assistant-mcp",version="1.9.0",on_list_tools=list_tools,on_call_tool=call_tool)
async def main():
